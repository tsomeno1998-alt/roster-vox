import { getTranslations } from 'next-intl/server';
import { Link } from '@/i18n/navigation';
import { signInWithGoogle, signInWithDiscord } from '@/app/actions/auth';
import EmailAuthForm, { EMAIL_ERRORS } from '@/components/auth/EmailAuthForm';
import Button from '@/components/ui/Button';

interface Props {
  searchParams: Promise<{ error?: string }>;
}

export default async function LoginPage({ searchParams }: Props) {
  const { error } = await searchParams;
  const t = await getTranslations('auth');
  const isOAuthError = error && !EMAIL_ERRORS.includes(error);

  return (
    <div className="min-h-screen bg-surface flex flex-col items-center justify-center px-6 py-12">
      <div className="mb-8 text-center">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/logo.svg" alt="Roster Vox" className="w-20 h-20 mx-auto mb-4" />
        <h1 className="text-2xl font-bold text-tx">{t('title')}</h1>
        <p className="text-sm text-tx-muted mt-1">{t('subtitle')}</p>
      </div>

      {isOAuthError && (
        <div className="w-full max-w-xs mb-4 px-4 py-3 rounded-xl bg-red-50 border border-red-200 text-sm text-red-600 text-center">
          {t('errorOAuth')}
        </div>
      )}

      <div className="w-full max-w-xs flex flex-col gap-3 mb-6">
        <form action={signInWithGoogle}>
          <Button type="submit" variant="secondary" fullWidth size="lg">
            <GoogleIcon />
            {t('googleLogin')}
          </Button>
        </form>
        <form action={signInWithDiscord}>
          <Button type="submit" fullWidth size="lg" className="bg-[#5865F2] hover:bg-[#4752C4] text-white">
            <DiscordIcon />
            {t('discordLogin')}
          </Button>
        </form>
      </div>

      <div className="w-full max-w-xs flex items-center gap-3 mb-6">
        <div className="flex-1 h-px bg-bd" />
        <span className="text-xs text-tx-light">{t('orSeparator')}</span>
        <div className="flex-1 h-px bg-bd" />
      </div>

      <EmailAuthForm error={error} />

      <p className="mt-8 text-xs text-tx-light text-center max-w-xs">
        {t.rich('terms', {
          termsLink: (chunks) => <Link href="/terms" className="underline hover:text-primary">{chunks}</Link>,
          privacyLink: (chunks) => <Link href="/privacy" className="underline hover:text-primary">{chunks}</Link>,
        })}
      </p>
    </div>
  );
}

function GoogleIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24">
      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
      <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
    </svg>
  );
}

function DiscordIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="white">
      <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057c.002.022.015.043.03.056a19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03z" />
    </svg>
  );
}
